// src/pages/driver/DriverChatPage.jsx
// Driver Chat — แชทกับ Buyer (order ที่รับอยู่) + Admin
// Room types: driver_admin
    ...
    return (
      <div className="empty-state" style={{ paddingTop: 60 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--color-text-hint)', marginTop: 6 }}>
          ห้องแชทจะเกิดขึ้นเมื่อรับงานส่งของ
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      {rooms.map((room) => {
        ...
        const isDriverSeller = room.room_type === 'driver_seller';
        const name           = isAdmin
          ? 'Admin Support'
          : isDriverSeller
            ? (room.seller_name || 'ร้านค้า')
            : (room.buyer_name || room.other_name || room.name || 'ผู้สั่งซื้อ');
        const avatar   = isAdmin
          ? null
          : isDriverSeller
            ? toImgUrl(room.seller_image)
            : toImgUrl(room.buyer_avatar || room.other_avatar);
        const lastMsg  = room.last_message || '';
        const unread   = room.unread_driver || room.unread_count || 0;
        ...
        return (
          ...
          >
            ...
            <div style={{
              ...
              background: isAdmin ? '#e8eaf6' : isDriverSeller ? '#f0faf5' : '#fff3e0',
              flexShrink: 0,
...
/* ── Chat Window ─────────────────────────────────── */
function ChatWindow({ room, driverProfile, socket, onBack }) {
  const { t } = useTranslation();
  ...
  // ── Seen indicator ──
  // driver_admin → otherType = 'admin', driver_seller → otherType = 'seller'
  const isAdmin        = room.room_type?.includes('admin');
  const isDriverSeller = room.room_type === 'driver_seller';
  const otherType      = isAdmin ? 'admin' : 'seller';
  const initUnread     = isAdmin ? (room.unread_admin || 0) : (room.unread_seller || 0);
  const [seenByOther, setSeenByOther] = useState(initUnread === 0);

  const roomName = isAdmin
    ? 'Admin Support'
    : isDriverSeller
      ? (room.seller_name || 'ร้านค้า')
      : (room.buyer_name || room.other_name || room.name || 'ผู้สั่งซื้อ');
  ...
  const loadMessages = useCallback(async () => {
    try {
      const res = await driverApi.get(`/chat/rooms/${room.id}/messages`, { params: { limit: 50 } });
      const msgs = res.data.messages || res.data.data || res.data || [];
      // Bug fix: API คืนผล old→new แล้ว ไม่ต้อง reverse ซ้ำ
  ...
  useEffect(() => {
    loadMessages();
    driverApi.post(`/chat/rooms/${room.id}/read`).catch(() => {});
  ...
  async function handleSend() {
    ...
    // Mode 1: แก้ไขข้อความ (ไม่ reset seen — ไม่ใช่ข้อความใหม่)
    if (editingMsg) {
      const newText = text.trim();
      if (!newText) return;
      setSending(true);
      try {
        const res = await driverApi.put(`/chat/rooms/${room.id}/messages/${editingMsg.id}`, { message: newText });
    ...
    if (imageFile) {
      ...
      try {
        ...
        await driverApi.post(`/chat/rooms/${room.id}/messages`, form, {
    ...
    setSending(true);
    try {
      await driverApi.post(`/chat/rooms/${room.id}/messages`, { message: txt });
      setText('');
      setSeenByOther(false);
      loadMessages();
    } catch { /* silent */ }
  ...
  async function handleDeleteMsg(msgId) {
    ...
    try {
      await driverApi.delete(`/chat/rooms/${room.id}/messages/${msgId}`);
      setMessages((prev) => prev.filter((m) => m.id !== msgId));
    } catch { /* silent */ }
  }

  const myId = driverProfile?.id;
  const hasExtra = imagePreview || editingMsg;

  // หา id ของข้อความล่าสุดที่ฉัน (driver) ส่ง — ใช้แสดง seen indicator
  let lastSentMsgId = null;
  for (let i = messages.length - 1; i >= 0; i--) {
    if (messages[i].sender_type === 'driver') { lastSentMsgId = messages[i].id; break; }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div className="top-bar">
        ...
        <span className="top-bar-title" style={{ fontSize: 14 }}>{roomName}</span>
        <div style={{ width: 32 }} />
      </div>
      ...
      <div style={{
        ...
        {loading ? (
          ...
          messages.map((msg) => {
            const isMine  = msg.sender_id === myId || msg.sender_type === 'driver';
...
/* ── Main DriverChatPage ─────────────────────────── */
export default function DriverChatPage({ socket, onUnreadChange }) {
  const { t } = useTranslation();
  ...
  // Load driver profile
  useEffect(() => {
    driverApi.get('/drivers/profile')
      .then((r) => setProfile(r.data.driver || r.data.data || r.data))
      .catch(() => {});
  }, []);
  ...
  const loadRooms = useCallback(async () => {
    setLoading(true);
    try {
      const res = await driverApi.get('/chat/rooms');
      const list = res.data.rooms || res.data.data || res.data || [];
      setRooms(Array.isArray(list) ? list : []);
      const totalUnread = list.reduce((sum, r) => sum + (r.unread_driver || r.unread_count || 0), 0);
  ...
  /* ── Contact Admin — สร้างหรือเปิดห้อง driver_admin ── */
  async function handleContactAdmin() {
    try {
      const r = await driverApi.post('/chat/rooms', { type: 'driver_admin' });
      ...
      <RoomList
        ...
        onSelect={(room) => {
          ...
          setRooms((prev) => prev.map((r) => r.id === room.id
            ? { ...r, unread_driver: 0, unread_count: 0 } : r));
        }}
      />
    </div>
  );
